...
int main(int argc,char** argv)
{
	...
	while (motor)
	{
		...
		for (i = 0; i < motor->command_count; i++)
			printf("command[%i]: %s\n",i,motor->commands[i]);
		printf("stop_action count: %i\n",motor->stop_action_count);
		for (i = 0; i < motor->stop_action_count; i++)
			printf("stop_action[%i]: %s\n",i,motor->stop_actions[i]);
		printf("stop_action %i is choosen.\n",motor->stop_action);
		//Let's open the motors ;)
		ev3_open_motor(motor);
		motor = motor->next;
	}

	if ( motors == NULL | motors->next == NULL)
	{
		printf("Error: This test needs at least two motors connected!\n");
		return 1;
	}

	//Lets set the first motor to coast mode and the second one to stop:
	ev3_stop_action_motor_by_name( motors, "coast");
	ev3_stop_action_motor_by_name( motors->next, "brake");

	//Lets set the first motor to 30% and the second to 200 counts/s
	ev3_set_speed_sp( motors, 50 * motors->max_speed / 100);
